...
length_set = sprintf(...
    '\\setlength\\figurewidth{%.2f%s} \\setlength\\figureheight{%.2f%s}',...
    p.Results.fig_size(1), p.Results.fig_size_unit,...
    p.Results.fig_size(2), p.Results.fig_size_unit);
...
comp_cmd_str = sprintf(...
    'pdflatex.exe -synctex=1 -interaction=nonstopmode -jobname=%s """%s %s %s \\input{%s} """',...
    job_name, length_def, length_set, fn_set,...
    strrep(fullfile(which(p.Results.tex_wrapper_fn)), '\', '\\'));
...
if ~status
    ...
    if p.Results.cl_aux_files
        if exist(sprintf('%s.aux', job_name),'file')
            delete(sprintf('%s.aux', job_name))
        end
        if exist(sprintf('%s.log', job_name),'file')
            delete(sprintf('%s.log', job_name))
        end
        if exist(sprintf('%s.synctex.gz', job_name),'file')
            delete(sprintf('%s.synctex.gz', job_name))
        end
        if exist(sprintf('%s.out', job_name),'file')
            delete(sprintf('%s.out', job_name))
        end
    end
